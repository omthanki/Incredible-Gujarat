		...
		<div class="header">
		  ...
		  <form method="post" action="addhotel.php"  enctype="multipart/form-data">
			<div class="input-group">
		  	...
		  	<div class="input-group">
		  		<button type="submit" class="btn" name="addhotel">Add</button>
		  	</div>
		  </form>
		</div>
...
<?php
	...
	if(isset($_POST['addhotel']))
	{
		...
		$address = nl2br(addslashes(trim($_POST['address'])));
		...
		{		
			...
			if(in_array($img_ex_lc, $allowed_exs))
			{
				...
				$s = "SELECT * from hotel where Name = '$name' LIMIT 1";
				...
				elseif (ctype_alpha($number)) {
					echo '<script>alert("Contact number should not contain alphabets.")</script>';
				}
				elseif (ctype_punct($number)) {
					echo '<script>alert("Contact number should not contain symbols.")</script>';
				}
				...
				elseif (ctype_punct($city)) {
					...
					$query = "INSERT INTO hotel (Name, Image, Address, Contact, City) VALUES ('$name', '$img_upload_path', '$address', '$number', '$city')";
