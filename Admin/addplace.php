		...
		<div class="header">
		  ...
		  <form  action="" method="post" enctype="multipart/form-data">
		  	...
		  	<div class="input-group">
		  		...
		  		<select name="category" class="input-group" required>
		  	...
		  	<div class="input-group">
		  		<button type="submit" class="btn" name="addplace">Add</button>
		  	</div>
		  </form>
		</div>
...
<?php
	...
	if(isset($_POST['addplace']))
	{
		...
		$desc = nl2br(addslashes(trim($_POST['desc'])));
		...
		if ($error === 0)
		{
			...
			if(in_array($img_ex_lc, $allowed_exs))
			{
				...
				$placeimage = trim($img_upload_path);

				$img_name = $_FILES['location']['name'];
				$img_size = $_FILES['location']['size'];
				$tmp_name = $_FILES['location']['tmp_name'];
				...
				if ($error === 0)
				{
					...
					if(in_array($img_ex_lc, $allowed_exs))
					{
						...
						$locationimage = trim($img_upload_path);

						$s = "SELECT * from places where Title = '$title' LIMIT 1";
						...
						elseif (ctype_digit($title)) {
							echo '<script>alert("Title should not contain digits.")</script>';
						}
						elseif (ctype_punct($title)) {
							echo '<script>alert("Title should not contain symbols.")</script>';
						}
						...
						elseif (ctype_punct($city)) {
							...
							$query = "INSERT INTO places (Title, Category, Description, Images, Location, City) VALUES ('$title', '$category', '$desc', '$placeimage', '$locationimage', '$city')";
